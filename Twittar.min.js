...
var Twittar=new Class({Implements:[Options,Events],initialize:function(a,b,c){this.setOptions(c);this.username=a;this.element=$(b);this.url=this.formatUrl();this.request=new Request.JSONP({url:this.url,method:"get",onRequest:function(){this.fireEvent("request")}.bind(this),onComplete:this.buildTweets.bind(this)})},options:{twitterUrl:"https://api.twitter.com/1/statuses/user_timeline.json?&exclude_replies={replies}&contributor_details={contributerDetails}&include_rts={retweets}&screen_name={username}&count={count}",
